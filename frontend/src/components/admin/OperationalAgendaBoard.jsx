...
function EventCard({ appointment, attendanceStatus, top, height, onOpen, onDragStart }) {
  const start = new Date(appointment.startTime);
  ...
  return (
    <button
      ...
      title={draggable ? 'Abrir atendimento. No desktop, arraste para reagendar.' : 'Abrir atendimento.'}
    >
...
export function OperationalAgendaBoard({ appointments, professionals, reload, setPage }) {
  ...
  const [operationalData, setOperationalData] = useState({ role: '', services: [], attendanceByAppointment: {} });
  ...
  async function loadOperationalOptions() {
    try {
      ...
      setMessage(error.message || 'Não foi possível carregar as ações da agenda operacional.');
    }
  }
  ...
  const revenuePotential = selectedDayAppointments
  ...
  async function updateAttendance(status) {
    ...
    setSaving(true);
    try {
      await request(`/admin/appointments/${selectedAppointment.id}/attendance`, {
        method: 'PUT',
        body: JSON.stringify({ status })
      });
    ...
    <main className="ops-agenda-page">
      <div className="ops-shell">
        <header className="ops-header">
          <div>
            <span className="eyebrow">Agenda operacional 2.0</span>
            <h1>O dia inteiro em uma única visão</h1>
            <p>Arraste atendimentos para reagendar e clique em um espaço livre para agendar ou bloquear. A linha vermelha mostra o horário atual.</p>
          </div>
                        ...
                        {professionalAppointments.map((appointment) => {
                          ...
                          return (
                            <EventCard
                              ...
                              attendanceStatus={operationalData.attendanceByAppointment[appointment.id] || 'SCHEDULED'}
                              top={position.top}
      ...
      {quickContext && (
        <div className="ops-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setQuickContext(null); }}>
          <section className="ops-modal ops-quick-modal" role="dialog" aria-modal="true" aria-label="Ação rápida na agenda">
            <header>
              <div><span className="eyebrow">Ação rápida</span><h2>{quickContext.time} · {quickContext.professional.name}</h2><p>{dateLabel(quickContext.date)}</p></div>
              <button type="button" onClick={() => setQuickContext(null)} aria-label="Fechar">×</button>
            </header>

            <div className="ops-quick-tabs">
              <button type="button" className={quickMode === 'BOOKING' ? 'active' : ''} onClick={() => setQuickMode('BOOKING')}>+ Novo agendamento</button>
              <button type="button" className={quickMode === 'BLOCK' ? 'active' : ''} onClick={() => setQuickMode('BLOCK')}>Bloquear horário</button>
            </div>
        ...
        <div className="ops-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedAppointment(null); }}>
          <section className="ops-modal" role="dialog" aria-modal="true" aria-label="Detalhes do atendimento">
            ...
            {selectedAppointment.status === 'CONFIRMED' && (
              <div className="ops-status-actions ops-attendance-actions">
                <strong>Fluxo do atendimento</strong>
                <div>
                  <button type="button" className={selectedAttendance === 'SCHEDULED' ? 'is-current' : ''} onClick={() => updateAttendance('SCHEDULED')} disabled={saving}>Confirmado</button>
                  <button type="button" className={selectedAttendance === 'ARRIVED' ? 'is-current arrived' : ''} onClick={() => updateAttendance('ARRIVED')} disabled={saving}>✓ Cliente chegou</button>
                  <button type="button" className={selectedAttendance === 'IN_SERVICE' ? 'is-current service' : ''} onClick={() => updateAttendance('IN_SERVICE')} disabled={saving}>▶ Em atendimento</button>
                </div>
              </div>
            )}
      ...
      <style>{`
        ...
        @media(max-width:560px){.ops-agenda-page{padding:22px 8px 60px}.ops-header-actions>*{flex:1}.ops-date-nav{display:grid;grid-template-columns:42px 1fr auto 42px}.ops-date-nav label{min-width:0}.ops-date-nav input{width:100%}.ops-kpis{gap:6px}.ops-kpis article{padding:11px}.ops-board-wrap{border-radius:16px}.ops-time-rail{width:48px;min-width:48px}.ops-time-body>span{right:5px;font-size:.62rem}.ops-lane{min-width:210px}.ops-modal{padding:16px;border-radius:18px}.ops-modal-summary{grid-template-columns:1fr}.ops-move-form,.ops-quick-form{grid-template-columns:1fr}.ops-move-form>*,.ops-quick-form>*{grid-column:1/-1}.ops-quick-tabs{grid-template-columns:1fr}}
      `}</style>
    </main>
  );
}
