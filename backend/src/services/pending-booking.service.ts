...
async function lastOutbound(salonId: string, clientPhone: string) {
...
export async function confirmPendingBooking(input: { salonId: string; salonName: string; clientPhone: string; clientName?: string; text: string }) {
  ...
  const summary = parseSummary(await lastOutbound(input.salonId, input.clientPhone));
  ...
  if (existing) return { handled: true, appointmentId: existing.id, replyText: `Seu agendamento já está confirmado ✅\n\n${service.name}\n📅 ${display(start)}\nProfissional: ${professional.name}` };
